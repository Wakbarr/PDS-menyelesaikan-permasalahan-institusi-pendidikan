...
import plotly.graph_objects as intestinal
...
def predict_student(data_dict, model, scaler, class_names):
    ...
    try:
        ...
        result = {
            ...
            'probabilities': {class_name: probabilities[i] for i, class_name in enumerate(class_names)}
        }
        return result
...
def create_gauge_chart(confidence, prediction):
    ...
    fig = intestinal.Figure(intestinal.Indicator(
...
def main():
    ...
    with tab1:
        with st.sidebar.form(key='prediction_form'):
            ...
            with col1:
                age = st.number_input("Umur saat mendaftar", min_value=16, max_value=50, value=18, help="Umur mahasiswa saat mendaftar")
                gender = st.selectbox("Gender", options=[1, 2], format_func=lambda x: "Female" if x == 1 else "Male", help="Pilih gender mahasiswa")
                marital_status = st.selectbox("Status Pernikahan", options=[1, 2, 3, 4, 5, 6], 
                                            format_func=lambda x: ["Single", "Married", "Widower", "Divorced", "Facto union", "Legally separated"][x-1], help="Status pernikahan")
            with col2:
                nationality = st.number_input("Kewarganegaraan (ID)", min_value=1, max_value=100, value=1, help="ID kewarganegaraan")
                international = st.selectbox("Mahasiswa Internasional", options=[0, 1], format_func=lambda x: "No" if x == 0 else "Yes", help="Status mahasiswa internasional")
                displaced = st.selectbox("Status Pengungsi", options=[0, 1], format_func=lambda x: "No" if x == 0 else "Yes", help="Status pengungsi")
            
            st.subheader("Data Akademik")
            admission_grade = st.number_input("Nilai Masuk", min_value=0.0, max_value=200.0, value=127.0, help="Nilai masuk universitas")
            prev_qualification_grade = st.number_input("Nilai Kualifikasi Sebelumnya", min_value=0.0, max_value=200.0, value=160.0, help="Nilai sebelumnya")
            course = st.number_input("ID Course", min_value=1, max_value=200, value=33, help="ID course yang diambil")
            daytime_attendance = st.selectbox("Waktu Kuliah", options=[1, 0], format_func=lambda x: "Siang" if x == 1 else "Malam", help="Waktu kuliah")
            
            st.subheader("Data Finansial & Beasiswa")
            tuition_up_to_date = st.selectbox("SPP Up to Date", options=[0, 1], format_func=lambda x: "No" if x == 0 else "Yes", help="Status SPP")
            debtor = st.selectbox("Status Debitur", options=[0, 1], format_func=lambda x: "No" if x == 0 else "Yes", help="Status debitur")
            scholarship = st.selectbox("Penerima Beasiswa", options=[0, 1], format_func=lambda x: "No" if x == 0 else "Yes", help="Status beasiswa")
            
            st.subheader("Performa Semester 1")
            sem1_enrolled = st.number_input("Unit Semester 1 Terdaftar", min_value=0, max_value=30, value=0, help="Unit terdaftar semester 1")
            sem1_approved = st.number_input("Unit Semester 1 Lulus", min_value=0, max_value=30, value=0, help="Unit lulus semester 1")
            sem1_grade = st.number_input("Nilai Rata-rata Semester 1", min_value=0.0, max_value=20.0, value=0.0, help="Nilai rata-rata semester 1")
            
            st.subheader("Performa Semester 2")
            sem2_enrolled = st.number_input("Unit Semester 2 Terdaftar", min_value=0, max_value=30, value=0, help="Unit terdaftar semester 2")
            sem2_approved = st.number_input("Unit Semester 2 Lulus", min_value=0, max_value=30, value=0, help="Unit lulus semester 2")
            sem2_grade = st.number_input("Nilai Rata-rata Semester 2", min_value=0.0, max_value=20.0, value=0.0, help="Nilai rata-rata semester 2")
            
            st.subheader("Data Ekonomi")
            unemployment_rate = st.number_input("Tingkat Pengangguran (%)", min_value=0.0, max_value=50.0, value=10.8, help="Tingkat pengangguran")
            inflation_rate = st.number_input("Tingkat Inflasi (%)", min_value=-10.0, max_value=20.0, value=1.4, help="Tingkat inflasi")
            gdp = st.number_input("GDP", min_value=-5.0, max_value=10.0, value=1.74, help="GDP saat ini")
            
            submit_button = st.form_submit_button(label="🔮 Prediksi Sekarang!", type="primary")
            ...
            if result:
                ...
                with col1:
                    if result['prediction'] == 'Dropout':
                        ...
                        """, unsafe_allow_html=True)
                        st.warning("🚨 **Perhatian!** Mahasiswa ini memiliki risiko dropout yang tinggi. Disarankan untuk memberikan dukungan akademik dan konseling.")
                    else:
                        st.markdown(f"""
                        ...
                        """, unsafe_allow_html=True)
                        st.success("🎉 **Bagus!** Mahasiswa ini memiliki kemungkinan tinggi untuk menyelesaikan studinya dengan baik.")
        ...
        if uploaded_file is not None:
            try:
                ...
                if st.button("🚀 Jalankan Batch Prediction"):
                    ...
                    fig.update_layout(title="Distribusi Prediksi Batch", template="plotly_dark")
